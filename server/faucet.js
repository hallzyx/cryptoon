...
/**
 * request ETH from CDP Faucet (for gas fees)
 */
export async function requestEthFaucet(address, apiKeyId, apiKeySecret) {
  // generate JWT for auth
  const jwt = await generateJwt({
    apiKeyId: apiKeyId,
    apiKeySecret: apiKeySecret,
  ...
  const response = await fetch('https://api.cdp.coinbase.com/platform/v2/evm/faucet', {
    ...
    body: JSON.stringify({
      network: 'base-sepolia',
      address: address,
      token: 'eth',  // Request ETH instead of USDC
    }),
  });

  if (!response.ok) {
    const errorText = await response.text();
    console.error("ETH Faucet API error:", errorText);
    throw new Error(`ETH Faucet request failed: ${response.status} ${errorText}`);
  }
